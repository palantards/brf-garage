...
export default async function ResignPage({ params }: Props) {
  ...
  const [resignation] = await sql<{
    ...
    SELECT r.email, r.resident_name, r.spot_identifier, r.agreement_type,
           a.name AS association_name
    FROM resignations r
    JOIN associations a ON a.id = r.association_id
    WHERE r.token = ${token}
      AND r.status = 'form_sent'
  `;
  ...
  return (
    <ResignationForm
      ...
      residentName={resignation.resident_name}
      spotIdentifier={resignation.spot_identifier}
      agreementType={resignation.agreement_type}
      associationName={resignation.association_name}
    />
  );
}
